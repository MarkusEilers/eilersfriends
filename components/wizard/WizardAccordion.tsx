...
const STEPS = [
  ...
  { key: '08-letzten-20-prozent', voice: 'Die letzten 20 Prozent', title: 'Name · Headline · CTA' },
]
          ...
          <li key={s.key} className={`rounded-2xl border ${isOpen ? 'border-blue-300 shadow-sm' : done ? 'border-green-200' : 'border-gray-200'} bg-white overflow-hidden`}>
            ...
            {isOpen && (
              <div className="px-5 pb-5">
                ...
                {s.key === '04-phasen-waehrung' && <PhasenWaehrungStep initialAnswers={stepAnswers as Parameters<typeof PhasenWaehrungStep>[0]['initialAnswers']} onSaved={handleSaved} />}
                {s.key === '05-beweis-stapel' && <BeweisStapelStep initialAnswers={stepAnswers as Parameters<typeof BeweisStapelStep>[0]['initialAnswers']} onSaved={handleSaved} />}
                ...
                {s.key === '07-wort-garantie' && <WortGarantieStep initialAnswers={stepAnswers as Parameters<typeof WortGarantieStep>[0]['initialAnswers']} onSaved={handleSaved} />}
                {s.key === '08-letzten-20-prozent' && <Letzten20Step initialAnswers={stepAnswers as Parameters<typeof Letzten20Step>[0]['initialAnswers']} onSaved={handleSaved} />}
              </div>
            )}
          </li>
        )
