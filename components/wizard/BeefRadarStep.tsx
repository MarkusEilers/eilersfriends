...
export function BeefRadarStep({ initialAnswers, onSaved }: Props) {
  ...
  async function suggest() {
    ...
    try {
      ...
      if (!res.ok || !data.result) {
        setError(data.error || 'Suggest fehlgeschlagen.'); setStatus('error'); return
      }
  ...
  function addCard(column: 'what' | 'how' | 'why') {
    setCards((prev) => [...prev, { column, text: '' }])
  }

  const grouped: Record<'what' | 'how' | 'why', { card: Card; idx: number }[]> = {
    what: [], how: [], why: [],
  }
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6">
        ...
        <textarea
          ...
          placeholder={'Was tut Dein Angebot? Welches Problem löst es? Wie verändert es den Tag des Kunden?"
        />
        ...
        <div className='}mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm focus:border-blue-400 focus:outline-none"
              placeholder={'z.B. 9.997 € Setup + 297 €/MA/Mo"
            />
          </div>
        </div>
        ...
        <div className='}mt-5 flex flex-wrap items-center gap-3">
          <button
            onClick={suggest}
            disabled={status === 'suggesting' || !offerDescription.trim()}
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6">
          <div className="mb-4">
            ...
            <p className="mt-1 text-xs text-gray-500">
              Editier die Karten direkt — was die AI vorgeschlagen hat, ist nur ein Startpunkt.
            </p>
          </div>
              ...
              return (
                <div key={col} className="rounded-xl border" style={{ borderColor: meta.color + '40', backgroundColor: meta.bg }}>
                  <div className="flex items-center justify-between p-3 border-b" style={{ borderColor: meta.color + '40' }}>
                    ...
                    <button onClick={() => addCard(col)} className="rounded-full p-1 hover:bg-white" title="Karte hinzufügen">
                      <Plus size={12} style={{ color: meta.color }} />
                    </button>
                  </div>
                  <ul className="p-3 space-y-2">
                    {items.length === 0 && (
                      <li className="text-xs text-gray-400 italic">Noch keine Karte.</li>
                    )}
                    {items.map(({ card, idx }) => (
                      <li key={idx} className="rounded-lg bg-white p-2.5 border border-white/60">
                        ...
                        {card.detail !== undefined && (
                          <input
                            ...
                            placeholder={'(Detail)"
                            className='}mt-1.5 w-full bg-transparent text-[10px] text-gray-600 focus:outline-none"
                          />
                        )}
                      </li>
                    ))}
          ...
          <div className="mt-5 flex items-center gap-3">
            <button
              onClick={save}
              disabled={status === 'saving' || cards.length === 0}
