...
const COUNTRY_KEYS: [string, string][] = [['Deutschland','de'],['Österreich','at'],['Schweiz','ch'],['Niederlande','nl'],['Frankreich','fr'],['Andere EU','eu']]
...
export function CheckoutForm({ programSlug, programName, tiers, enrollmentLimit, enrollmentDeadline, freeSeatPer = 0, bonusTiers = [], cities, maxSeats = 200, guaranteeText, guaranteeBadge }: Props) {
  ...
  const billingSuffix = (b: CheckoutTier['billing']) => b === 'monthly' ? tr('perMonth') : b === 'yearly' ? tr('perYear') : tr('oneTime')
  ...
  if (!selectedTier) return <p className="text-sm text-red">{tr('noTiers')}</p>
  ...
  if (freeSeats >= 1) bonuses.push(`${freeSeats} ${freeSeats === 1 ? tr('freeSeatOne') : tr('freeSeatMany')}`)
  ...
  async function submit() {
    ...
    try {
      ...
      if (!res.ok || !data.url) { setError(data.error || tr('errorSession')); return }
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-lg">
      ...
      <div className="border-b border-gray-100 bg-cream px-6 py-5">
        <p className="mb-3 text-[10px] font-bold uppercase tracking-widest text-gray-500">{tr('yourSelection')}</p>
        ...
        {enrollmentLimit && (
          <p className="mt-4 text-[11px] text-muted">{tr('seatsLimit',{n:enrollmentLimit})}{deadlineDate ? ` · ${tr('seatsLimitDeadline',{n:enrollmentLimit,date:deadlineDate})}` : ''}</p>
        )}
      </div>

      {maxSeats > 1 && (
      <div className="border-b border-gray-100 px-6 py-5">
        <p className="mb-3 text-[10px] font-bold uppercase tracking-widest text-gray-500">{tr('seatCount')}</p>
        <div className="flex items-center gap-4">
          <div className="inline-flex items-center rounded-xl border border-gray-200">
            <button type="button" onClick={() => setSeats((v) => Math.max(1, v - 1))} disabled={seats <= 1}
              className="px-3 py-2 text-lg font-bold text-ink disabled:opacity-30" aria-label={tr('less')}>−</button>
            <span className="w-10 text-center font-bold text-ink">{seats}</span>
            <button type="button" onClick={() => setSeats((v) => Math.min(maxSeats, v + 1))}
              className="px-3 py-2 text-lg font-bold text-ink" aria-label={tr('more')}>+</button>
          </div>
          <span className="text-xs text-muted">{tr('seatsPayable',{paid:paidSeats})}{freeSeats > 0 ? ` · ${tr('seatsFree',{free:freeSeats})}` : ''}</span>
        </div>
        {bonuses.length > 0 && (
          <div className="mt-4 rounded-xl bg-blue-bg p-3">
            <p className="mb-2 text-[10px] font-bold uppercase tracking-widest text-blue">{tr('yourBonus')}</p>
      ...
      <div className="space-y-4 px-6 py-5">
        <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500">{tr('yourData')}</p>

        {cities && cities.length > 0 && (
          <Field label={tr('dateCity')}>
        ...
        <Field label={tr('company')}>
          <input type="text" value={company} onChange={(e) => setCompany(e.target.value)}
            placeholder={tr('companyPlaceholder')} className="input-base" />
        </Field>
        ...
        <Field label={tr('country')}>
          <select value={country} onChange={(e) => setCountry(e.target.value)} className="input-base">
            {COUNTRY_KEYS.map(([v,k]) => <option key={v} value={v}>{tr('countries.'+k)}</option>)}
          </select>
        </Field>
        ...
        <div className="space-y-1 rounded-xl bg-cream p-4 text-sm">
          <div className="flex justify-between text-muted">
            <span>{programName} · {selectedTier.label} × {paidSeats}{freeSeats > 0 ? ` (+${tr('seatsFree',{free:freeSeats})})` : ''}</span>
            <span>{formatEUR(subtotal)}</span>
          </div>
          <div className="flex justify-between text-muted">
            <span>{isVatExempt ? tr('vatReverse') : tr('vat19')}</span>
        ...
        <label className="flex items-start gap-2 text-xs text-gray-700">
          ...
          <span>{tr('acceptPre')} <a href="/agb" target="_blank" className="text-blue underline">{tr('terms')}</a> {tr('and')} <a href="/datenschutz" target="_blank" className="text-blue underline">{tr('privacy')}</a>.{guaranteeText ? ` ${tr('guaranteeApplies',{guarantee:guaranteeText})}` : ''}</span>
        </label>
        ...
        >
          ...
          {submitting ? tr('redirecting') : tr('payNow')}
        </button>

        <div className="flex items-center justify-center gap-2 pt-1 text-[10px] text-muted">
          <span>{tr('secure')}</span><span>·</span>
          <span>{tr('stripeCheckout')}</span>{guaranteeBadge ? <><span>·</span><span>{guaranteeBadge}</span></> : null}
        </div>
      </div>
