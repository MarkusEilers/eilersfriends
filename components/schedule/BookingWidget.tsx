...
function dayHuman(key: string) {
  ...
  return new Intl.DateTimeFormat('de-DE', { timeZone: TZ, weekday: 'long', day: 'numeric', month: 'long' }).format(new Date(Date.UTC(y, m - 1, d, 12)))
}
...
export function BookingWidget({ person, type, personName, durationMin, infoText, questions, hosts }: {
  ...
  useEffect(() => {
    ...
    fetch(`/api/schedule/availability?person=${person}&type=${type}`).then(r => r.json()).then(d => {
      ...
      setSlots(s)
      if (s.length) { const first = berlinDayKey(s[0]); setDayKey(first); const [y, m] = first.split('-').map(Number); setCursor({ y, m }) }
  ...
  async function submit() {
    ...
    for (const q of qs) if (q.required && !(answers[q.id] || '').trim()) { setErr(`Bitte beantworten: „${q.label}"`); return }
    ...
    try {
      ...
      if (!res.ok) {
        if (d.error === 'slot_taken' || d.error === 'day_full') { setErr('Diese Zeit ist gerade vergeben — bitte eine andere wählen.'); setSlot(null); setSlots(prev => prev.filter(s => s !== slot)) }
        else if (d.error === 'missing_answer') setErr('Bitte alle Pflichtfelder ausfüllen.')
        else setErr('Buchung fehlgeschlagen. Bitte erneut versuchen.')
        return
      }
      setBooked({ start: d.start || slot, end: d.end || slot, joinUrl: d.joinUrl || null, title: d.title || `Termin mit ${personName}`, manageUrl: d.manageUrl || null })
  ...
  if (!slots.length) return <>{intro}<Info>Aktuell sind im Buchungsfenster keine freien Zeiten. Schreib uns: <a href="mailto:team@eilersfriends.com" className="font-semibold underline" style={{ color: ACCENT }}>team@eilersfriends.com</a>.</Info></>
    ...
    <>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <div className="mb-2 grid grid-cols-7 gap-1 text-center text-[11px] font-semibold uppercase tracking-wide text-gray-400">{WD.map(w => <div key={w}>{w}</div>)}</div>
          ...
          {!slot ? (
            <>
              ...
              <div className="grid max-h-[320px] grid-cols-3 gap-2 overflow-y-auto pr-1 sm:grid-cols-4">
                {dayTimes.map(s => <button key={s} onClick={() => setSlot(s)} className="rounded-lg border border-gray-200 px-2 py-2.5 text-sm font-semibold text-gray-700 transition-colors hover:border-blue-400 hover:bg-blue-50">{timeLabel(s)}</button>)}
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Nachricht (optional)" rows={2} className="w-full resize-none rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-blue-400" />
              </div>
...
function Done({ booked, email, personName, durationMin }: { booked: Booked; email: string; personName: string; durationMin: number }) {
  ...
  return (
    <div className="rounded-2xl border border-green-200 bg-green-50 p-8 text-center">
      ...
      {booked.joinUrl && <a href={booked.joinUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 rounded-full bg-green-700 px-5 py-2.5 text-sm font-bold text-white hover:bg-green-800"><Video size={15} /> Teams-Meeting öffnen</a>}
      ...
      {booked.manageUrl && <p className="mt-4 text-xs text-green-800">Umbuchen oder absagen: <a href={booked.manageUrl} className="font-semibold underline">{booked.manageUrl}</a></p>}
    </div>
  )
}
