import { PERSONS, TYPES, WORK } from '@/lib/schedule/config'
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">Kalender-Verbindungen, Termin-Typen und Verfügbarkeits-Regeln für die Online-Buchung.</p>
      </div>

      {sp.connected && (
        <div className="mb-6 rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">Kalender für <strong>{sp.connected}</strong> verbunden.</div>
      )}
      {sp.error && (
        <div className="mb-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">Verbindung fehlgeschlagen ({sp.error}). Bitte erneut versuchen.</div>
      )}
          ...
          return (
            <div key={p.slug} className="rounded-2xl border border-gray-100 bg-white p-5">
              ...
              <div className="mt-4 flex items-center justify-between">
                ...
                <a
                  href={configured ? `/api/schedule/oauth/start?person=${p.slug}` : undefined}
      ...
      {/* Types */}
      <h2 className="mb-3 mt-8 text-sm font-bold uppercase tracking-widest text-gray-400">Termin-Typen</h2>
      <div className="grid gap-3 sm:grid-cols-2">
        {TYPES.map((t) => (
          <div key={t.slug} className="rounded-2xl border border-gray-100 bg-white p-5">
            <div className="flex items-center justify-between">
              <p className="text-sm font-bold text-gray-900">{t.name}</p>
              <span className="rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-bold text-blue-700">{t.durationMin} Min</span>
            </div>
            <p className="mt-1 text-xs text-gray-500">{t.description}</p>
          </div>
        ))}
      </div>
      <p className="mt-4 text-xs text-gray-400">Arbeitszeiten: Mo–Fr {WORK.startHour}:00–{WORK.endHour}:00 ({WORK.tz}), {WORK.bufferMin} Min Puffer, {WORK.leadHours} h Vorlauf, {WORK.horizonDays} Tage Horizont. Typen/Zeiten später hier editierbar.</p>
    </div>
  )
}
