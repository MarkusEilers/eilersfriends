...
export default async function ManagePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const title = et ? `${et.name} ${t('with')} ${ent?.name || ''}` : `${t('with')} ${ent?.name || ''}`
  return (
    <div style={{ backgroundColor: '#FAFAF8' }}>
      <section className="px-6 py-14 sm:py-20">
        <div className="mx-auto max-w-xl">
          <p className="text-xs font-bold uppercase tracking-widest" style={{ color: '#1A5FD4' }}>{t('yourAppointment')}</p>
          <h1 className="mt-2 text-3xl font-bold sm:text-4xl" style={{ color: '#0D0D0B' }}>{title}</h1>
          <div className="mt-6">
            <ManageWidget token={token} owner={b.ownerSlug} type={b.typeSlug} personName={ent?.name || ''} start={b.startUtc}
              durationMin={et?.durationMin || Math.round((new Date(b.endUtc).getTime() - new Date(b.startUtc).getTime()) / 60000)} status={b.status} joinUrl={b.joinUrl} />
          </div>
        </div>
      </section>
    </div>
