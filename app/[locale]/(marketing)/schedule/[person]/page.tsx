    ...
    <div style={{ backgroundColor: '#FAFAF8' }}>
      <section className="px-6 py-16 sm:py-20">
        <div className="mx-auto max-w-3xl">
          <Link href="/schedule" className="text-xs font-semibold text-gray-400 hover:text-gray-700">← {t('all')}</Link>
          <h1 className="mt-3 text-4xl font-bold sm:text-5xl" style={{ color: '#0D0D0B' }}>{ent.name}</h1>
          <p className="mt-2 text-base text-gray-600">{t('whichType')}</p>
          {types.length === 0 ? (
            <div className="mt-8 rounded-2xl border border-gray-200 bg-white p-6 text-sm text-gray-600">{t('noTypes')} <a href="mailto:team@eilersfriends.com" className="font-semibold underline" style={{ color: '#1A5FD4' }}>team@eilersfriends.com</a>.</div>
          ) : (
